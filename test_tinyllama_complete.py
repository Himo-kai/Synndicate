...
from synndicate.observability.logging import (
    setup_logging, get_logger, set_trace_id, get_trace_id, clear_trace_id
)
from synndicate.observability.probe import probe, get_trace_metrics
...
async def test_tinyllama_integration():
    ...
    try:
        ...
        print(f"  📊 Existing models: {len(model_manager._embedding_models)} embedding, {len(model_manager._language_models)} language")
        ...
        with probe("test.tinyllama_loading", test_trace_id):
            logger.info("Loading TinyLlama model", model="tinyllama-1.1b-chat", path=str(tinyllama_path))
...
async def test_complete_system_workflow():
    ...
    try:
        ...
        with probe("test.complete_system_workflow", e2e_trace_id):
            logger.info("Starting complete system workflow test", query_length=len(test_query.strip()))
            
            result = await orchestrator.process_query(
                test_query.strip(),
                context={"trace_id": e2e_trace_id},
        ...
        if result.response_text:
            snippet = result.response_text[:300] + "..." if len(result.response_text) > 300 else result.response_text
    ...
    try:
        ...
        for name, prompt, max_tokens in scenarios:
            with probe(f"perf.{name.lower().replace(' ', '_')}", perf_trace_id):
                ...
                print(f"  🚀 {name}: {duration:.2f}s, {words_per_second:.1f} words/s, {words} words")
    ...
    try:
        ...
        if success1 and success2 and success3:
            ...
            if success3 and perf_results:
                avg_words_per_sec = sum(r["words_per_second"] for r in perf_results) / len(perf_results)
