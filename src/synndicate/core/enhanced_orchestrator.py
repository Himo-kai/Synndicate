...
from ..agents.dynamic_coder import DynamicCoderAgent
from ..agents.dynamic_critic import DynamicCriticAgent
...
class EnhancedOrchestrator(Orchestrator):
    ...
    async def _execute_dynamic_workflow(
        ...
        try:
            ...
            dismissed_count = await self.dynamic_orchestrator.dismiss_agents(task_id, recruited_agents)
    ...
    async def _execute_collaborative_workflow(
        ...
        if AgentRole.PLANNER in agents and AgentRole.CODER in agents:
            ...
            result = OrchestratorResult(
                ...
                agents_used=[r.value for r in agents.keys()],
                execution_path=["single_agent"],
                confidence=response.confidence,
                metadata={
                    "agent_count": len(agents),
                    "recruited_roles": [role.value for role in agents.keys()],
    ...
    async def _execute_plan_and_code_workflow(
        ...
        if AgentRole.CRITIC in agents:
            ...
            review_response = await critic.process(
                "Review the implemented code for quality and improvements",
                review_context
            )
            
            # Combine responses
            final_response.response = f"{code_response.response}\n\n## Code Review:\n{review_response.response}"
        ...
        result = OrchestratorResult(
            ...
            execution_time=time.time() - start_time,
            agents_used=[r.value for r in agents.keys()],
            execution_path=["planning", "coding"] + (["review"] if AgentRole.CRITIC in agents else []),
    ...
    async def _execute_plan_and_review_workflow(
        ...
        final_response.response = f"{plan_response.response}\n\n## Plan Review:\n{review_response.response}"
