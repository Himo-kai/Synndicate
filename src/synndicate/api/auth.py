...
class RateLimiter:
    ...
    def is_rate_limited(self, request: Request, tier: RateLimitTier, api_key: Optional[str] = None) -> Tuple[bool, Dict[str, Any]]:
        ...
        if requests_in_minute >= rate_config.burst_size:
            ...
            if recent_requests >= rate_config.burst_size:
                log.warning("Rate limit exceeded (burst)", client=client_key, requests=recent_requests)
                return True, {
        ...
        if requests_in_minute >= rate_config.requests_per_minute:
            log.warning("Rate limit exceeded (per minute)", client=client_key, requests=requests_in_minute)
            return True, {
    ...
    def record_failed_attempt(self, request: Request):
        ...
        log.warning("Failed authentication attempt", ip=client_ip, attempts=self.failed_attempts[client_ip])
        ...
        if self.failed_attempts[client_ip] >= self.config.max_failed_attempts:
            ...
            log.error("IP blocked due to repeated failures", 
                     ip=client_ip, 
...
class AuthManager:
    ...
    async def authenticate_request(self, request: Request) -> Tuple[Optional[APIKey], RateLimitTier]:
...
def require_auth(min_role: UserRole = UserRole.READONLY):
    ...
    def decorator(func):
        ...
        async def wrapper(request: Request, *args, **kwargs):
            ...
            if api_key and UserRole(api_key.role).value < UserRole(min_role).value:
                raise HTTPException(
            ...
            if is_limited:
                raise HTTPException(
                    ...
                    headers={
                        ...
                        "X-RateLimit-Limit": str(auth_manager.config.rate_limits[tier].requests_per_minute),
...
def add_rate_limit_headers(response, request: Request):
    ...
    if hasattr(request.state, "rate_limit_info"):
        ...
        response.headers["X-RateLimit-Limit"] = str(auth_manager.config.rate_limits[tier].requests_per_minute)
