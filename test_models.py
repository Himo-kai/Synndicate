...
async def test_embedding_model(manager: ModelManager):
...
async def test_language_model(manager: ModelManager):
...
async def test_health_check(manager: ModelManager):
    ...
    try:
        ...
        if health_status['language_models']:
            ...
            for name, status in health_status['language_models'].items():
                print(f"    - {name}: {'✅' if status['healthy'] else '❌'} (loaded: {status['loaded']})")
...
async def test_rag_integration(manager: ModelManager):
    ...
    try:
        ...
        print(f"  ✅ Generated embeddings for RAG chunks")
        print(f"  ✅ Embedding compatibility confirmed")
