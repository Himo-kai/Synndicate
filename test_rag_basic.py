...
from src.synndicate.rag.chunking import SemanticChunker, ChunkType
from src.synndicate.rag.indexer import DocumentIndexer
from src.synndicate.rag.retriever import RAGRetriever, QueryContext
...
async def test_context_integration():
    ...
    for agent_type in ['planner', 'coder', 'critic']:
        ...
        print(f"  ✅ Formatted context for {agent_type} agent ({agent_context.total_tokens} tokens)")
...
''')
        ...
        for query, task_type in test_queries:
            ...
            print(f"    Context: {agent_context.chunk_count} chunks, {agent_context.total_tokens} tokens")
            print(f"    Top result: {results[0].chunk.content[:60]}..." if results else "    No results")
