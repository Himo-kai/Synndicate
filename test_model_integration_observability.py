...
from synndicate.observability.probe import probe, get_trace_metrics, clear_trace_metrics
    ...
    try:
        ...
        with probe("test.model_fallback", fallback_trace_id):
            try:
                ...
                print(f"  ⚠️  Unexpected success with non-existent model")
        ...
        with probe("test.model_health_checks", health_trace_id):
            ...
            for name, model in model_manager._language_models.items():
                try:
                    ...
                    print(f"  🤖 Language model {name}: {'✅ HEALTHY' if health else '❌ UNHEALTHY'}")
                except Exception as e:
            ...
            for name, model in model_manager._embedding_models.items():
                try:
                    ...
                    print(f"  🔢 Embedding model {name}: {'✅ HEALTHY' if health else '❌ UNHEALTHY'}")
                except Exception as e:
...
async def main():
    ...
    try:
        ...
        print("=" * 50)
        print(f"  ✅ Model manager observability: PASS")
        print(f"  ✅ Orchestrator-model integration: PASS")
        print(f"  ✅ Model fallback behavior: PASS")
        print(f"  ✅ Model health checks: PASS")
        
        total_operations = (
            len(mm_metrics) + len(e2e_metrics) + 
            len(fallback_metrics) + len(health_metrics)
        )
