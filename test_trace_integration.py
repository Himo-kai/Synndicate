    ...
    for agent_name in agents:
        with probe(f"flow.{agent_name}", flow_trace_id):
            ...
            if agent_name == "coder" and "planner" in results:
                ...
                context["plan"] = results.get("planner", {}).response if "planner" in results else ""
...
async def main():
    ...
    try:
        ...
        print("=" * 40)
        print(f"  ✅ Orchestrator tracing: PASS")
        print(f"  ✅ Agent tracing: PASS")
        print(f"  ✅ Multi-agent flow: PASS")
        print(f"  ✅ Trace isolation: PASS")
        
        total_operations = (
            len(orch_metrics) + len(agent_metrics) + 
            len(flow_metrics) + len(metrics_a) + len(metrics_b)
        )
