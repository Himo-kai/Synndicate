...
from synndicate.rag.context import ContextBuilder, ContextStrategy
...
class SynndacateDemo:
    ...
    async def _setup_openai_model(self):
        ...
        try:
            config = ModelConfig( name="gpt-3.5-turbo", model_type=ModelType.LANGUAGE_MODEL, format=ModelFormat.OPENAI_API, path="gpt-3.5-turbo", parameters={
    ...
    async def _setup_knowledge_base(self):
        ...
        # Setup RAG retriever
        if self.model_manager is not None:
            self.rag_retriever = RAGRetriever( chunks=all_chunks, embedding_function=lambda texts: self.model_manager.generate_embeddings(texts) if self.model_manager else [],
            )
    ...
    async def demonstrate_rag_retrieval(self, query: str):
        ...
        if self.rag_retriever is not None:
            results = await self.rag_retriever.retrieve(query_context, SearchMode.HYBRID, max_results=3)
        else:
            results = []
        ...
        context = context_builder.build_context(
            results, max_tokens=300, strategy=ContextStrategy.CONCATENATE
        )
    ...
    async def demonstrate_language_generation(self, prompt: str, context: str = ""):
        ...
        if self.model_manager is not None:
            ...
            has_models = loaded_models["language_models"]
        else:
            has_models = []
...
"""
        ...
        try:
            if self.model_manager is not None:
                response = await self.model_manager.generate_text(
                    full_prompt, config=GenerationConfig(max_tokens=200, temperature=0.7)
                )
            else:
                return None
    ...
    async def demonstrate_agent_workflow(self, task: str):
        ...
        planning_response = await self.demonstrate_language_generation(
            f"Create a step-by-step plan to {task}", planning_context.content
        )
        ...
        impl_response = await self.demonstrate_language_generation(
            f"Implement the solution for: {task}", impl_context.content
        )

        # Step 3: Review (using RAG for best practices)
        print("\n🔍 Step 3: Review Phase")
        review_context, _ = await self.demonstrate_rag_retrieval(
            "Python best practices code review"
        )
        review_response = await self.demonstrate_language_generation(
            "Review the implementation and suggest improvements", review_context.content
        )
    ...
    async def run_demo(self):
        ...
        await self.demonstrate_agent_workflow(
        ...
        if self.model_manager is not None:
            ...
            loaded = {'embedding_models': [], 'language_models': []}
