...
from synndicate.api.auth import (
    APIKey, AuthManager, RateLimiter, SecurityConfig, UserRole, RateLimitTier,
    require_auth, get_auth_manager, add_rate_limit_headers
)
...
def auth_manager(security_config):
    ...
    manager.add_api_key("readonly_key_12345", UserRole.READONLY, RateLimitTier.READONLY, "Test Readonly")
...
class TestRateLimiting:
    ...
    def test_rate_limit_per_minute_exceeded(self, security_config, mock_request):
        ...
        for i in range(61):
            history.append(current_time - i * 0.5)  # Spread over 30 seconds
    ...
    def test_different_tier_limits(self, security_config, mock_request):
        ...
        is_limited_readonly, info_readonly = limiter.is_rate_limited(mock_request, RateLimitTier.READONLY)
...
class TestAuthenticationDecorator:
    ...
    def test_require_auth_decorator_success(self, auth_manager, mock_request):
        ...
        with patch('synndicate.api.auth.get_auth_manager', return_value=auth_manager):
            with patch.object(auth_manager, 'authenticate_request', return_value=(
                auth_manager.config.api_keys[auth_manager._hash_key("user_key_12345")],
                RateLimitTier.USER
            )):
                with patch.object(auth_manager.rate_limiter, 'is_rate_limited', return_value=(False, {"requests_remaining": 59})):
    ...
    def test_require_auth_decorator_missing_key(self, auth_manager, mock_request):
        ...
        with patch('synndicate.api.auth.get_auth_manager', return_value=auth_manager):
            with patch.object(auth_manager, 'authenticate_request', return_value=(None, RateLimitTier.ANONYMOUS)):
    ...
    def test_require_auth_decorator_insufficient_role(self, auth_manager, mock_request):
        ...
        with patch('synndicate.api.auth.get_auth_manager', return_value=auth_manager):
            with patch.object(auth_manager, 'authenticate_request', return_value=(user_key, RateLimitTier.USER)):
                with pytest.raises(HTTPException) as exc_info:
                    import asyncio
                    asyncio.run(test_endpoint(mock_request))
    ...
    def test_require_auth_decorator_rate_limited(self, auth_manager, mock_request):
        ...
        with patch('synndicate.api.auth.get_auth_manager', return_value=auth_manager):
            with patch.object(auth_manager, 'authenticate_request', return_value=(user_key, RateLimitTier.USER)):
                with patch.object(auth_manager.rate_limiter, 'is_rate_limited', return_value=(True, {
...
class TestRateLimitHeaders:
    ...
    def test_add_rate_limit_headers(self, mock_request):
        ...
        with patch('synndicate.api.auth.get_auth_manager') as mock_get_auth:
            mock_auth = MagicMock()
            mock_auth.config.rate_limits = {
                RateLimitTier.USER: MagicMock(requests_per_minute=60)
            }
