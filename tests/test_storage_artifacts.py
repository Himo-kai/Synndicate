...
class TestArtifactRef:
    ...
    def test_artifact_ref_creation(self):
        ...
        ref = ArtifactRef(
            uri="file:///tmp/test.txt", backend="local", size_bytes=1024, content_type="text/plain"
        )
...
class TestS3Store:
    ...
    def test_s3_delete(self, mock_boto3_client):
        ...
        mock_client.delete_object.assert_called_once_with(Bucket="test-bucket", Key="delete_me.txt")
...
class TestConvenienceFunctions:
    ...
    def test_save_trace_snapshot(self, mock_get_store):
        ...
        mock_store.save_json.assert_called_once_with(f"traces/trace_{trace_id}.json", snapshot)
    ...
    def test_save_performance_data(self, mock_get_store):
        ...
        mock_store.save_text.assert_called_once_with(
            f"perf/perf_{trace_id}.jsonl", '{"metric": "cpu", "value": 0.8}'
        )
    ...
    def test_save_audit_data(self, mock_get_store):
        ...
        assert result == mock_ref
        mock_store.save_json.assert_called_once_with(f"audit/{filename}", data)
