...
class IntegrationTestRunner:
    ...
    async def test_performance_under_load(self) -> Dict[str, Any]:
        ...
        try:
            ...
            failed_results = [r for r in results if isinstance(r, Exception) or not getattr(r, 'success', False)]
            
            avg_execution_time = sum(r.execution_time for r in successful_results) / len(successful_results) if successful_results else 0
            avg_confidence = sum(r.response.confidence for r in successful_results if r.response) / len(successful_results) if successful_results else 0
    ...
    async def test_resource_optimization(self) -> Dict[str, Any]:
        ...
        try:
            orchestrator = DynamicOrchestrator(max_agents=3, idle_timeout=1.0)  # Short timeout for testing
            ...
            for agent_id in recruited_agents:
                if agent_id in orchestrator.agents:
                    orchestrator.agents[agent_id].status = orchestrator.agents[agent_id].status.__class__.IDLE
                    orchestrator.agents[agent_id].metrics.last_used = time.time() - 2.0  # 2 seconds ago
    ...
    async def test_real_coding_workload(self) -> Dict[str, Any]:
        ...
        try:
            ...
            quality_indicators = {
                ...
                "has_error_handling": any(keyword in response_text for keyword in ["try:", "except:", "raise"]),
    ...
    async def test_plan_and_review_workflow(self) -> Dict[str, Any]:
        ...
        try:
            ...
            has_plan = any(keyword in response_text.lower() for keyword in ["plan", "strategy", "approach", "steps"])
            has_review = any(keyword in response_text.lower() for keyword in ["review", "analysis", "assessment", "feedback"])
...
async def main():
    ...
    try:
        ...
        if summary["success_rate"] >= 80:
            logger.info("🎉 Integration tests PASSED! Dynamic orchestration system is ready for production.")
        else:
